...
window.buildContractHTML = (task) => {
    ...
    /* Strict, hardcoded, non-editable merchant exceptions. Legal text must never
       be customizable from the UI, so these are keyed purely off the merchant
       name at generation time. */
    const merchantNameKey = String(merchantName || '').replace(/\s+/g, ' ').trim();

    const financialSpecialException = merchantNameKey.includes('سفير السعادة')
    ...
    clauses.push(
...

        {
            ...
            body: `يتم تنظيم الدورة المالية بين الطرفين على النحو التالي: في الطلبات النقدية (الدفع عند الاستلام) يلتزم مندوب كانجو بدفع قيمة الطلب إلى الطرف الثاني عند الاستلام، وتصبح عمولة كانجو دينًا مستحقًا على الطرف الثاني يظهر بشكل مباشر وحي على التطبيق، ويتم تحصيلها دوريًا قبل بلوغ الحد الائتماني المقرر. وفي الطلبات الإلكترونية (الدفع أونلاين) تصبح كانجو مدينة للطرف الثاني بقيمة الطلب بعد خصم عمولتها، وتظهر المستحقات بشكل مباشر وحي على التطبيق، ويمكن للطرف الثاني طلب سحب مستحقاته في أي وقت، وتُحوَّل إليه في أسرع وقت ممكن وفقًا لقواعد وتعليمات البنك المركزي المصري.${financialSpecialException}<br><br>ملاحظات:<div style="height: 30px; border-bottom: 1px dashed #000; margin-bottom: 8px; width: 100%; margin-top: 6px;"></div><div style="height: 30px; border-bottom: 1px dashed #000; margin-bottom: 8px; width: 100%;"></div><div style="height: 30px; border-bottom: 1px dashed #000; margin-bottom: 8px; width: 100%;"></div><div style="height: 30px; border-bottom: 1px dashed #000; margin-bottom: 8px; width: 100%;"></div><div style="height: 30px; border-bottom: 1px dashed #000; margin-bottom: 8px; width: 100%;"></div>`
